...
(function injectSNSStyles() {
    ...
    style.textContent = `
        ...
        .sns-feed-track { display: flex; gap: 14px; padding: 4px 2px; width: max-content; }
        .sns-feed-card { position: relative; width: 200px; flex-shrink: 0; background: white; border-radius: 1rem; border: 1px solid #f3f4f6; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.06); transition: transform 0.2s, box-shadow 0.2s; cursor: pointer; }
        .dark .sns-feed-card { background: #1e293b; border-color: #334155; }
        @media (min-width: 640px) { .sns-feed-card:hover { transform: translateY(-4px); box-shadow: 0 8px 24px rgba(0,0,0,0.12); } }
        .sns-feed-card.type-yt { border-top: 3px solid #FF0000; }
        .sns-feed-card.type-ig { border-top: 3px solid #E4405F; }
        .sns-feed-thumb { width: 100%; height: 120px; object-fit: cover; display: block; background: #f1f5f9; }
        .dark .sns-feed-thumb { background: #0f172a; }
        .sns-feed-thumb-placeholder { width: 100%; height: 120px; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #f8fafc, #f1f5f9); color: #cbd5e1; font-size: 2rem; }
        .dark .sns-feed-thumb-placeholder { background: linear-gradient(135deg, #1e293b, #0f172a); }
        .sns-feed-info { padding: 10px 12px 12px; }
        .sns-feed-candidate { font-size: 0.65rem; font-weight: 900; color: #FF6600; margin-bottom: 3px; display: flex; align-items: center; gap: 4px; }
        .sns-feed-title { font-size: 0.7rem; font-weight: 700; color: #1e293b; line-height: 1.35; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; min-height: 1.9em; }
        .dark .sns-feed-title { color: #e2e8f0; }
        .sns-feed-type-badge { position: absolute; top: 8px; right: 8px; padding: 2px 7px; border-radius: 6px; font-size: 0.55rem; font-weight: 900; color: white; letter-spacing: 0.03em; }
        ...
        .sns-feed-empty { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 2.5rem 1rem; color: #cbd5e1; gap: 10px; font-size: 0.8rem; font-weight: 700; }
        .dark .sns-feed-empty { color: #475569; }
        .sns-feed-loading { display: flex; align-items: center; justify-content: center; padding: 2rem; gap: 8px; color: #94a3b8; font-size: 0.8rem; font-weight: 700; }
        @keyframes snsPulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.4; } }
        .sns-pulse { animation: snsPulse 1.2s ease-in-out infinite; }
        .sns-feed-date { font-size: 0.6rem; font-weight: 600; color: #94a3b8; margin-top: 4px; }
        .dark .sns-feed-date { color: #64748b; }
        /* 채널 링크카드 */
        .sns-channel-card { width: 130px; flex-shrink: 0; background: white; border-radius: 1rem; border: 1px solid #f3f4f6; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.05); transition: transform 0.2s, box-shadow 0.2s; cursor: pointer; display: flex; flex-direction: column; align-items: center; padding: 14px 10px 12px; gap: 8px; text-align: center; }
        .dark .sns-channel-card { background: #1e293b; border-color: #334155; }
        @media (min-width: 640px) { .sns-channel-card:hover { transform: translateY(-4px); box-shadow: 0 8px 20px rgba(0,0,0,0.10); } }
        ...
        .sns-channel-avatar { width: 48px; height: 48px; border-radius: 50%; object-fit: cover; background: #f1f5f9; display: block; }
        .dark .sns-channel-avatar { background: #0f172a; }
        .sns-channel-avatar-placeholder { width: 48px; height: 48px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.4rem; flex-shrink: 0; }
        ...
        .sns-channel-name { font-size: 0.7rem; font-weight: 900; color: #1e293b; line-height: 1.3; word-break: keep-all; }
        .dark .sns-channel-name { color: #e2e8f0; }
        .sns-channel-sub { font-size: 0.6rem; font-weight: 600; color: #94a3b8; }
        .sns-channel-badge { display: inline-flex; align-items: center; gap: 3px; padding: 2px 7px; border-radius: 99px; font-size: 0.55rem; font-weight: 900; color: white; }
        ...
        /* ── 2줄 임베드 카드 ── */
        .sns-embed-card { flex-shrink: 0; display: flex; flex-direction: column; border-radius: 1rem; overflow: hidden; box-shadow: 0 2px 12px rgba(0,0,0,0.10); background: #000; }
        .sns-embed-card.type-yt  { border-top: 3px solid #FF0000; }
        .sns-embed-card.type-ig  { border-top: 3px solid #E4405F; }
        .sns-embed-label { display: flex; align-items: center; gap: 6px; padding: 6px 10px; background: #fff; min-height: 30px; }
        .dark .sns-embed-label { background: #1e293b; }
        .sns-embed-badge { font-size: 0.55rem; font-weight: 900; color: #fff; padding: 2px 6px; border-radius: 4px; flex-shrink: 0; }
        ...
        .sns-embed-candidate { font-size: 0.65rem; font-weight: 900; color: #FF6600; flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .sns-embed-link { font-size: 0.7rem; color: #94a3b8; text-decoration: none; flex-shrink: 0; }
        .sns-embed-link:hover { color: #FF6600; }
        /* 커스텀 스크롤바 */
        .sns-custom-scrollbar-wrap { padding: 0 2px 8px; margin-bottom: 6px; }
        .sns-custom-scrollbar-track { height: 4px; background: #f1f5f9; border-radius: 99px; position: relative; cursor: pointer; }
        .dark .sns-custom-scrollbar-track { background: #1e293b; }
        .sns-custom-scrollbar-thumb { position: absolute; top: 0; height: 4px; border-radius: 99px; background: #FF6600; cursor: grab; transition: background 0.15s; min-width: 30px; }
        .sns-custom-scrollbar-thumb:hover, .sns-custom-scrollbar-thumb.dragging { background: #e55a00; cursor: grabbing; }
        /* iframe 드래그 오버레이 */
        .sns-embed-card { position: relative; }
        .sns-embed-drag-overlay { position: absolute; inset: 30px 0 0 0; z-index: 10; cursor: grab; }
...
async function loadRow1() {
    ...
    try {
        const res = await fetch('https://api.restep.cc/videos'); // https 적용 시 이걸로 변경 권장
        ...
        const allVideos = data
            ...
            .filter(item => !hiddenNames.has(item.candidate_name))
            .map(item => ({
        ...
        const scrollWrap = document.getElementById('sns-row1-track')?.parentElement;
        if (scrollWrap) initDragScroll(scrollWrap);
        
    } catch (error) {
        console.error("영상 로드 에러:", error);
        track.innerHTML = `
...
function initCustomScrollbar(scrollWrap, thumbEl, trackEl) {
    ...
    function updateThumb() {
        ...
        if (scrollWidth <= clientWidth) { thumbEl.style.display = 'none'; return; }
    ...
    thumbEl.addEventListener('mousedown', e => {
        isDragging = true; startX = e.clientX; startScrollLeft = scrollWrap.scrollLeft;
        thumbEl.classList.add('dragging'); e.preventDefault(); e.stopPropagation();
    });
    document.addEventListener('mousemove', e => {
        ...
        const scrollDx = ((e.clientX - startX) / maxThumbX) * maxScroll;
        scrollWrap.scrollLeft = Math.max(0, Math.min(maxScroll, startScrollLeft + scrollDx));
    });
    document.addEventListener('mouseup', () => {
        if (!isDragging) return;
        isDragging = false; thumbEl.classList.remove('dragging');
    });

    trackEl.addEventListener('click', e => {
        if (e.target === thumbEl) return;
        const ratio = (e.clientX - trackEl.getBoundingClientRect().left) / trackEl.clientWidth;
        scrollWrap.scrollLeft = ratio * (scrollWrap.scrollWidth - scrollWrap.clientWidth);
    });

    thumbEl.addEventListener('touchstart', e => {
        isDragging = true; startX = e.touches[0].clientX; startScrollLeft = scrollWrap.scrollLeft;
    ...
    document.addEventListener('touchmove', e => {
        ...
        const maxThumbX = trackEl.clientWidth - thumbEl.clientWidth;
        scrollWrap.scrollLeft = Math.max(0, Math.min(maxScroll, startScrollLeft + ((e.touches[0].clientX - startX) / maxThumbX) * maxScroll));
    }, { passive: true });
...
function initDragScroll(el) {
    ...
    const setOverlayPointer = (active) => {
        overlays().forEach(o => { o.style.pointerEvents = active ? 'auto' : 'none'; o.classList.toggle('dragging', active); });
    };
